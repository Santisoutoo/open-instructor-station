...
| Whether a spawned aircraft actually trips the *aircraft's own* TCAS instrument | Depends on X-Plane's/the aircraft's own TCAS implementation picking up the TCAS-target entries the bridge will publish — the §10.4 spike ruled out the multiplayer slots as the mechanism and left TCAS-target *write* behaviour unverified. This manager's job ends at "the geometry is realistic and the entity is really there." |
...
- Spawn a `tcas_conflict` (`head_on_ra`) ahead of the live aircraft; poll the bridge's own
  `ois/traffic/contacts` dataref until it reports a position matching the spawned track; despawn,
  assert the contact clears. **Not** `sim/multiplayer/position/plane*`: the §10.4 spike measured
  that those slots never populate under XPLMInstance driving — an empty slot reads the −3.4e38
  sentinel, so polling them would wait forever on a correctly-working bridge.
- Once Track B's TCAS-target write path exists, additionally assert the read-back of the entity's
  TCAS-target entry (`sim/cockpit2/tcas/targets/*`) — the one `-m sim` assertion §10.4's findings
  make a precondition for re-baselining `MAX_CONCURRENT_TRAFFIC_XPLANE` to the 64-entry table.
...
- `test_spawn_capacity_is_enforced`'s live variant is explicitly marked slow/opt-in — spawning a
  capacity-full set of real traffic entities is not something to do on every `-m sim` run.
...
**min 23.3 ms, median 26.9 ms, worst 36.6 ms**, and the ack was there on the
...
(§3.2 field names), is **1 572 bytes** and round-tripped intact in 25.5 ms (byte-exactness rests
on the SHA-verified `echo` series — the spawn ack's own hash was computed over a JSON
re-serialisation in this run; the spike plugin has since been corrected to hash the received
bytes); the spawned entity then appeared in `ois/traffic/contacts` (read as a *variable-length*
data dataref — that style works too) and `despawn` emptied it. **§5.3's chunking fallback is not needed.** The one
...
**TCAS-target override API** is **present and writable — write behaviour unverified**:
`sim/operation/override/override_TCAS` and `sim/cockpit2/tcas/targets/modeS_id` (int array,
**size 64**) both report `writable = true`, but the spike never wrote a byte to either, and this
same session's multiplayer-slot result is the standing proof that `writable = true` does not
imply a write sticks. Consequence for Track B: the bridge drives entities as **XPLMInstance
objects for the visual** — one mechanism for any `.obj` the library resolves; only the tug was
actually instanced, so a suitable bird object still has to be found (the candidate bird paths
tried resolved to nothing), which leaves D12's "same mechanism" answer proven for ground
vehicles and *expected* for birds — **plus a TCAS-target entry via `override_TCAS`/`modeS_id`
for `kind="aircraft"`** so the student's TCAS can see them, the LiveTraffic/XPMP2 architecture.
§5.4's `MAX_CONCURRENT_TRAFFIC_XPLANE = 19` (multiplayer slots) should be re-baselined to the
TCAS-target budget (64 entries, one being the user aircraft) **only once Track B has exercised
a real TCAS-target write and asserted its read-back under `-m sim`** (§8.4) — until that
assertion exists, the number stays as designed; D6 — capacity is adapter-owned, the bridge is
the source of truth — is unchanged either way. Still open, and now clearly `@pytest.mark.sim`
territory for §8.4: the TCAS-target write behaviour itself, which aircraft `.obj`/CSL model to
instance for a convincing aircraft visual (this install ships `IVAO_CSL`), and whether the user
aircraft's own TCAS instrument actually trips on override-written targets (§1.2 already scoped
that out of this manager's exit criteria).
