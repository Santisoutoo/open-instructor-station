/**
 * A stubbed instructor API for the Failures panel's component tests.
 *
 * `fetch` is stubbed rather than the RTK Query hooks mocked — the same reasoning and the
 * same shape as `features/position/testApi.ts`: the request the panel actually sends is
 * the thing worth asserting, and mocking the hooks would hide a panel asking the wrong
 * endpoint. Duplicated rather than imported: it is one small file and importing across
 * feature folders would be the wrong coupling for two panels that otherwise share
 * nothing.
 *
