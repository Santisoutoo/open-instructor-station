/**
 * A stubbed instructor API for the Traffic panel's component tests.
 *
 * `fetch` is stubbed rather than the RTK Query hooks mocked — the same reasoning and the
 * same shape as `features/position/testApi.ts` and `features/failures/testApi.ts`: the
 * request the panel actually sends is the thing worth asserting. Duplicated rather than
 * imported: it is one small file and importing across feature folders would be the wrong
 * coupling for panels that otherwise share nothing.
 *
