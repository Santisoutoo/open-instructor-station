...
**CI never needs a simulator** (`AGENTS.md`). GitHub runners have no X-Plane and never will. Sim
...
[`AGENTS.md`](../../AGENTS.md) at the repository root is **binding**. The parts that bear on your
work:
...
For context, the full local verification set from `AGENTS.md` (you run only the sim one; the rest
